...
from ai.utils.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadScreenshots, LoadStreams2
from ai.utils.general import (LOGGER, Profile, check_file, check_img_size, check_imshow, check_requirements, colorstr,
                              cv2,
...
def run(
    ...
    if webcam:
        ...
        dataset = LoadStreams2(source, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
        bs = len(dataset)
    elif screenshot:
...
def parse_opt():
    ...
    # parser.add_argument('--source', type=str,
    #                     default='http://192.168.43.1:8081',
    #                     help='file/dir/URL/glob/screen/0(webcam)')
...
def main(opt):
    ...
    # points = [np.array([[20, 20], [1900, 20], [1900, 1080], [20, 1080]])]
    points = [np.array([[20, 20], [900, 20], [900, 1080], [20, 1080]])]

    run(**vars(opt), boundary=points)
